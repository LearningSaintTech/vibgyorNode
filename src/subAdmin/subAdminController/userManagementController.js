...
async function getAllUsers(req, res) {
	try {
		...
		// eslint-disable-next-line no-console
		console.log('[SUBADMIN][USER_MGMT] Users fetched successfully');
		...
		// eslint-disable-next-line no-console
		console.error('[SUBADMIN][USER_MGMT] getAllUsers error:', e?.message || e);
...
async function toggleUserStatus(req, res) {
	try {
		...
		// eslint-disable-next-line no-console
		console.log('[SUBADMIN][USER_MGMT] User status updated successfully');
		return ApiResponse.success(res, {
			userId: user._id,
		...
		// eslint-disable-next-line no-console
		console.error('[SUBADMIN][USER_MGMT] toggleUserStatus error:', e?.message || e);
...
async function getUserDetails(req, res) {
	try {
		...
		// eslint-disable-next-line no-console
		console.log('[SUBADMIN][USER_MGMT] User details fetched successfully');
		return ApiResponse.success(res, user);
	} catch (e) {
		// eslint-disable-next-line no-console
		console.error('[SUBADMIN][USER_MGMT] getUserDetails error:', e?.message || e);
...
async function getUserStats(req, res) {
	try {
		...
		const approvedVerifications = await User.countDocuments({ verificationStatus: 'approved' });

		// eslint-disable-next-line no-console
		console.log('[SUBADMIN][USER_MGMT] User stats fetched successfully');
		return ApiResponse.success(res, {
			totalUsers,
			activeUsers,
			inactiveUsers,
			...
			badgeVerificationRate: totalUsers > 0 ? Math.round((approvedVerifications / totalUsers) * 100) : 0
		});
	} catch (e) {
		// eslint-disable-next-line no-console
		console.error('[SUBADMIN][USER_MGMT] getUserStats error:', e?.message || e);
...
async function getPendingVerifications(req, res) {
	try {
		...
		// eslint-disable-next-line no-console
		console.log('[SUBADMIN][USER_MGMT] Pending verifications fetched successfully');
		return ApiResponse.success(res, {
			pendingVerifications,
			pagination: {
				page: parseInt(page),
		...
		// eslint-disable-next-line no-console
		console.error('[SUBADMIN][USER_MGMT] getPendingVerifications error:', e?.message || e);
...
async function approveUserVerification(req, res) {
	try {
		...
		// eslint-disable-next-line no-console
		console.log('[SUBADMIN][USER_MGMT] User verification approved successfully');
		return ApiResponse.success(res, {
			userId: user._id,
			username: user.username,
			fullName: user.fullName,
		...
		// eslint-disable-next-line no-console
		console.error('[SUBADMIN][USER_MGMT] approveUserVerification error:', e?.message || e);
...
async function rejectUserVerification(req, res) {
	try {
		// eslint-disable-next-line no-console
		console.log('[SUBADMIN][USER_MGMT] rejectUserVerification request');
		const { userId } = req.params || {};
		...
		// eslint-disable-next-line no-console
		console.log('[SUBADMIN][USER_MGMT] User verification rejected successfully');
		return ApiResponse.success(res, {
			userId: user._id,
			username: user.username,
			fullName: user.fullName,
		...
		// eslint-disable-next-line no-console
		console.error('[SUBADMIN][USER_MGMT] rejectUserVerification error:', e?.message || e);
...
async function getPendingReports(req, res) {
	try {
		...
		console.log('[SUBADMIN][USER_MGMT] Pending reports fetched successfully');
		return ApiResponse.success(res, {
			pendingReports,
			pagination: {
				page: parseInt(page),
		...
		console.error('[SUBADMIN][USER_MGMT] getPendingReports error:', e?.message || e);
...
async function updateReportStatus(req, res) {
	try {
		console.log('[SUBADMIN][USER_MGMT] updateReportStatus request');
		const { reportId } = req.params || {};
		...
		console.log('[SUBADMIN][USER_MGMT] Report status updated successfully');
		return ApiResponse.success(res, {
			reportId: report._id,
		...
		console.error('[SUBADMIN][USER_MGMT] updateReportStatus error:', e?.message || e);
...
async function getReportStats(req, res) {
	try {
		...
		console.log('[SUBADMIN][USER_MGMT] Report stats fetched successfully');
		return ApiResponse.success(res, {
			totalReports,
			recentReports,
			...stats
		...
		console.error('[SUBADMIN][USER_MGMT] getReportStats error:', e?.message || e);
