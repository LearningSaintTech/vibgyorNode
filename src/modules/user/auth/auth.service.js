...
async function updateProfile(req, res) {
	try {
		const body = req.body || {};
		const { dob, username, step } = body;
		const advanceStep = profileSteps.resolveAdvanceStep(body, step);
		const user = await User.findById(req.user?.userId);
		if (!user) return ApiResponse.notFound(res, 'User not found');
		...
		const profileBody = await mergeIconTextMediaIntoBody(
			body,
			req.profileUploadFiles || [],
			req.user?.userId
		);
		...
		if (!user.isProfileCompleted) {
			const stepResult = profileSteps.applyProfileStepUpdate(user, {
				explicitStep: step,
				body: profileBody,
				files: req.profileUploadFiles || [],
				advanceStep,
			});
			if (stepResult?.advanced === false && stepResult.code === 'REQUIRED_STEP_INCOMPLETE') {
				const blockedStep = profileSteps.normalizeProfileStep(step || user.profileCompletionStep);
				const message =
					blockedStep === profileSteps.PROFILE_STEPS.BIRTHDAY
						? 'Date of birth is required. You must be at least 18 years old.'
						: blockedStep === profileSteps.PROFILE_STEPS.SHORT_BIO
							? 'Short bio is required.'
							: blockedStep === profileSteps.PROFILE_STEPS.NAME_USERNAME
								? 'Full name and username are required.'
								: blockedStep === profileSteps.PROFILE_STEPS.LOOKING_FOR_DISTANCE
									? 'Please select at least one “looking for” option before continuing.'
									: blockedStep === profileSteps.PROFILE_STEPS.LOCATION
		...
		return ApiResponse.success(res, profileSteps.buildStepResponse(user), 'Profile updated');
	} catch (e) {
