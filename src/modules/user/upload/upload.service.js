const { User } = require('./upload.repository');
...
async function uploadIdProof(req, res) {
	try {
		...
		const fileFilter = (req, file, cb) => {
			if (!ACCEPTED_ID_PROOF_MIME.includes(file.mimetype)) {
		...
		const uploadMultipleFiles = multer({ 
			...
			fileFilter, 
			limits: {
				fileSize: MAX_ID_PROOF_FILE_SIZE,
				files: ID_PROOF_FILE_COUNT,
			},
		}).array('file', ID_PROOF_FILE_COUNT);
