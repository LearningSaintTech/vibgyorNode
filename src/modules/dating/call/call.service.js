const { DatingCall, DatingChat, User } = require('./call.repository');
const enhancedRealtimeService = require('../../../services/enhancedRealtimeService');
const notificationService = require('../../notification/services/notificationService');
const { v4: uuidv4 } = require('uuid');

/**
 * Enhanced Dating Call Service with comprehensive error handling and edge cases
 */
class DatingCallService {
  ...
  static async initiateCall(chatId, initiatorId, type = 'audio') {
    try {
      ...
      const existingCall = await DatingCall.findActiveCall(chatId);
      console.log('[DatingCallService] Checking for existing call in chat:', chatId);
      console.log('[DatingCallService] Existing call found:', !!existingCall);
      if (existingCall) {
        console.log('[DatingCallService] Existing call details:', {
        ...
        if (existingCall.startedAt < fiveMinutesAgo) {
          console.log('[DatingCallService] Existing call is stale, cleaning up...');
          // Directly update the call status to ended (bypassing endCall method restrictions)
          await DatingCall.findOneAndUpdate(
            { callId: existingCall.callId },
            {
              status: 'ended',
              endedAt: new Date(),
          ...
          console.log('[DatingCallService] Stale call cleaned up successfully');
      ...
      const recentCalls = await DatingCall.countDocuments({
      ...
      const callId = `dating_call_${Date.now()}_${uuidv4().substring(0, 8)}`;
      ...
      await call.save();
      console.log('[DatingCallService] New call created:', {
        callId: call.callId,
        chatId: call.chatId,
        status: call.status,
        initiator: initiatorId
      ...
      console.error('[DatingCallService] initiateCall error:', error);
      throw error;
    }
  }
  ...
  static async acceptCall(callId, userId, signalingData = {}) {
    try {
      ...
      // Find call
      const call = await DatingCall.findOne({ callId }).populate('chatId');
      if (!call) {
      ...
      console.error('[DatingCallService] acceptCall error:', error);
      throw error;
    }
  }
  ...
  static async rejectCall(callId, userId, reason = 'Call rejected') {
    try {
      ...
      console.error('[DatingCallService] rejectCall error:', error);
      throw error;
    }
  }
  ...
  static async endCall(callId, userId, reason = 'user_ended') {
    try {
      ...
      console.error('[DatingCallService] endCall error:', error);
      throw error;
    }
  }
  ...
  static async getCallStatus(callId, userId) {
    try {
      ...
      console.error('[DatingCallService] getCallStatus error:', error);
      throw error;
    }
  }
  ...
  static async getCallHistory(chatId, userId, options = {}) {
    try {
      ...
      const calls = await DatingCall.findUserCalls(userId, {
        ...queryOptions,
        chatId: chatId
      });
      ...
      console.error('[DatingCallService] getCallHistory error:', error);
      throw error;
    }
  }
  ...
  static async getCallStats(userId, dateRange = null) {
    try {
      ...
      const stats = await DatingCall.getCallStats(userId, dateRange);
      ...
      console.error('[DatingCallService] getCallStats error:', error);
      throw error;
    }
  }
  ...
  static async updateCallSettings(callId, userId, settings) {
    try {
      ...
      console.error('[DatingCallService] updateCallSettings error:', error);
      throw error;
    }
  }
  ...
  static async handleSignaling(callId, userId, signalingData) {
    try {
      console.log(`[DATING_CALL_SERVICE] 🔄 Processing WebRTC signaling:`, {
        callId,
        userId,
      ...
      if (!callId || !userId || !signalingData) {
        console.log(`[DATING_CALL_SERVICE] ❌ Validation failed: Missing required parameters`, {
          callId: !!callId,
          userId: !!userId,
      ...
      console.log(`[DATING_CALL_SERVICE] 📋 Signaling data received:`, {
        type,
        hasData: !!data,
      ...
      if (!['offer', 'answer', 'ice-candidate'].includes(type)) {
        console.log(`[DATING_CALL_SERVICE] ❌ Invalid signaling type:`, { type });
        throw new Error('Invalid signaling type');
      }
      
      // Find call
      console.log(`[DATING_CALL_SERVICE] 🔍 Looking up call in database:`, { callId });
      const call = await DatingCall.findOne({ callId });
      if (!call) {
        console.log(`[DATING_CALL_SERVICE] ❌ Call not found:`, { callId });
        throw new Error('Call not found');
      }
      
      console.log(`[DATING_CALL_SERVICE] ✅ Call found:`, {
        callId: call.callId,
        status: call.status,
      ...
      // Check if user is participant
      const isParticipant = call.participants.includes(userId);
      console.log(`[DATING_CALL_SERVICE] 🔍 Checking participant status:`, {
        userId,
        isParticipant,
      ...
      if (!isParticipant) {
        console.log(`[DATING_CALL_SERVICE] ❌ User is not a participant:`, { userId, callId });
        throw new Error('You are not a participant in this call');
      }
      
      console.log(`[DATING_CALL_SERVICE] ✅ User is authorized participant`);
      
      // Handle different signaling types
      console.log(`[DATING_CALL_SERVICE] 🔄 Processing ${type} signaling...`);
      switch (type) {
        case 'offer':
          console.log(`[DATING_CALL_SERVICE] 📤 Processing offer:`, {
            hasSDP: !!data.sdp,
            sdpLength: data.sdp?.length || 0,
            offerType: data.type,
          ...
          if (!data.sdp || !data.type) {
            console.log(`[DATING_CALL_SERVICE] ❌ Invalid offer data:`, { hasSDP: !!data.sdp, hasType: !!data.type });
            throw new Error('Invalid offer data');
          }
          
          if (!call.webrtcData) {
            console.log(`[DATING_CALL_SERVICE] 🔧 Initializing WebRTC data structure`);
          ...
          console.log(`[DATING_CALL_SERVICE] ✅ Offer stored in call data`);
          
          // Update call status to ringing
          if (call.status === 'initiating') {
            console.log(`[DATING_CALL_SERVICE] 🔄 Updating call status from ${call.status} to ringing`);
            call.status = 'ringing';
          } else {
            console.log(`[DATING_CALL_SERVICE] 📋 Call status remains: ${call.status}`);
          }
          break;
          
        case 'answer':
          console.log(`[DATING_CALL_SERVICE] 📥 Processing answer:`, {
            hasSDP: !!data.sdp,
            sdpLength: data.sdp?.length || 0,
          ...
          if (!data.sdp || !data.type) {
            console.log(`[DATING_CALL_SERVICE] ❌ Invalid answer data:`, { hasSDP: !!data.sdp, hasType: !!data.type });
            throw new Error('Invalid answer data');
          }
          
          if (!call.webrtcData) {
            console.log(`[DATING_CALL_SERVICE] 🔧 Initializing WebRTC data structure for answer`);
            call.webrtcData = { iceCandidates: [] };
          }
          ...
          console.log(`[DATING_CALL_SERVICE] ✅ Answer stored in call data`);
          break;
          
        case 'ice-candidate':
          console.log(`[DATING_CALL_SERVICE] 🧊 Processing ICE candidate:`, {
          ...
          if (!data.candidate) {
            console.log(`[DATING_CALL_SERVICE] ❌ Invalid ICE candidate data:`, { hasCandidate: !!data.candidate });
            throw new Error('Invalid ICE candidate data');
          }
          
          console.log(`[DATING_CALL_SERVICE] 🔄 Adding ICE candidate to call...`);
          await call.addIceCandidate(data);
          console.log(`[DATING_CALL_SERVICE] ✅ ICE candidate added successfully`);
          break;
      ...
      console.log(`[DATING_CALL_SERVICE] 💾 Saving call data to database...`);
      await call.save();
      console.log(`[DATING_CALL_SERVICE] ✅ Call data saved successfully`);
      ...
      console.log(`[DATING_CALL_SERVICE] ✅ Signaling processing completed:`, result);
      return result;
      
    } catch (error) {
      console.error(`[DATING_CALL_SERVICE] ❌ handleSignaling error:`, {
        error: error.message,
        stack: error.stack,
        callId,
        userId,
  ...
  static async getActiveCall(chatId, userId) {
    try {
      ...
      const activeCall = await DatingCall.findActiveCall(chatId);
      if (!activeCall) {
      ...
      console.error('[DatingCallService] getActiveCall error:', error);
      throw error;
    }
  }
  ...
  static async cleanupStaleCalls(maxAgeMinutes = 5) {
    try {
      return await DatingCall.cleanupStaleCalls(maxAgeMinutes);
    } catch (error) {
      console.error('[DatingCallService] cleanupStaleCalls error:', error);
      throw error;
    }
  }
  ...
  static async forceCleanupChatCalls(chatId, userId) {
    try {
      ...
      const calls = await DatingCall.find({
        chatId: chatId,
      ...
      console.error('[DatingCallService] forceCleanupChatCalls error:', error);
      throw error;
    }
  }
}

module.exports = DatingCallService;
