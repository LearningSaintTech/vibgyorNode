...
// Enhanced Dating Call Schema with comprehensive validation and edge cases
const datingCallSchema = new Schema({
  ...
  timestamps: true,
  versionKey: false,
  collection: 'datingcalls'
});

// Indexes for optimal performance
datingCallSchema.index({ initiator: 1, status: 1 });
datingCallSchema.index({ participants: 1, status: 1 });
datingCallSchema.index({ startedAt: -1 });
datingCallSchema.index({ type: 1, status: 1 });
datingCallSchema.index({ chatId: 1, status: 1 });
datingCallSchema.index({ 'webrtcData.offer.sdp': 1 });

// Pre-save middleware
datingCallSchema.pre('save', function(next) {
...
// Static methods
datingCallSchema.statics.findActiveCall = async function(chatId) {
  try {
...
datingCallSchema.statics.findUserCalls = async function(userId, options = {}) {
  try {
    const query = {
      $or: [
        { initiator: userId },
...
datingCallSchema.statics.getCallStats = async function(userId, dateRange = null) {
  try {
    const matchQuery = {
      $or: [
...
datingCallSchema.statics.cleanupStaleCalls = async function(maxAgeMinutes = 5) {
  try {
...
// Instance methods
datingCallSchema.methods.acceptCall = async function(acceptedBy) {
  try {
...
datingCallSchema.methods.rejectCall = async function(reason = 'Call rejected') {
  try {
...
datingCallSchema.methods.endCall = async function(endReason = 'user_ended', endedBy = null) {
  try {
...
datingCallSchema.methods.addIceCandidate = async function(candidateData) {
  try {
    if (!this.webrtcData) {
...
datingCallSchema.methods.updateCallQuality = async function(qualityData) {
  try {
    if (!this.quality) {
      this.quality = {};
    }
...
datingCallSchema.methods.logError = async function(errorType, message, severity = 'medium') {
  try {
    if (!this.errors) {
      this.errors = [];
    }
...
datingCallSchema.methods.updateSettings = async function(settings) {
  try {
...
// Virtual fields
datingCallSchema.virtual('calculatedDuration').get(function() {
...
datingCallSchema.virtual('isActive').get(function() {
  return ['initiating', 'ringing', 'connected'].includes(this.status);
});

datingCallSchema.virtual('otherParticipant').get(function() {
  return this.participants.find(p => p._id.toString() !== this.initiator.toString());
});

// Export model
const DatingCall = mongoose.model('DatingCall', datingCallSchema);

module.exports = DatingCall;
