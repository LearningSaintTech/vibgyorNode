const { User, Report, FollowRequest, Post } = require('./graph.repository');
	...
	try {
		...
		if (isPublicAccount) {
			...
			try {
				...
				console.log('[USER][SOCIAL] Γ£à Real-time events emitted: follow_status:updated to both users and profile rooms');
			} catch (realtimeError) {
		...
		try {
			...
			console.log('[USER][SOCIAL] Γ£à Real-time event emitted: follow_request:received to user', userId);
		} catch (realtimeError) {
	...
	try {
		...
		if (hasPendingRequest) {
			...
			if (deletedRequest) {
				try {
					...
					if (deleteResult.deletedCount > 0) {
						console.log('[USER][SOCIAL] Γ£à Successfully deleted follow request notification');
					} else {
						console.log('[USER][SOCIAL] ΓÜá∩╕Å No notification found to delete (may have been already deleted)');
					}
				} catch (notificationError) {
			...
			try {
				...
				console.log('[USER][SOCIAL] Γ£à Real-time event emitted: follow_request:cancelled to recipient');
			} catch (realtimeError) {
				console.error('[USER][SOCIAL] Error emitting real-time event:', realtimeError);
				// Don't fail the request if real-time event fails
			}
		...
		try {
			...
			if (pendingRequest) {
				...
				console.log('[USER][SOCIAL] Γ£à Real-time event emitted: follow_request:cancelled');
			}
			...
			console.log('[USER][SOCIAL] Γ£à Real-time events emitted: follow_status:updated to both users');
		} catch (realtimeError) {
...
async function removeFollower(req, res) {
	try {
		...
		try {
			...
			console.log('[USER][SOCIAL] Γ£à Real-time events emitted: follower:removed and follow_status:updated');
		} catch (realtimeError) {
			console.error('[USER][SOCIAL] Error emitting real-time events:', realtimeError);
			// Don't fail the request if real-time event fails
		}
	...
	try {
		...
		try {
			...
			if (updateResult.modifiedCount > 0) {
				console.log('[USER][SOCIAL] Γ£à Successfully updated follow_request notification status to accepted');
			} else {
				console.log('[USER][SOCIAL] ΓÜá∩╕Å No notification found to update (may have been already updated or deleted)');
			}
		...
		try {
			...
			console.log('[USER][SOCIAL] Γ£à Real-time events emitted: follow_request:accepted and follow_status:updated');
		} catch (realtimeError) {
			console.error('[USER][SOCIAL] Error emitting real-time events:', realtimeError);
			// Don't fail the request if real-time event fails
		}
...
async function rejectFollowRequest(req, res) {
	try {
		...
		try {
			...
			if (updateResult.modifiedCount > 0) {
				console.log('[USER][SOCIAL] Γ£à Successfully updated follow_request notification status to rejected');
			} else {
				console.log('[USER][SOCIAL] ΓÜá∩╕Å No notification found to update (may have been already updated or deleted)');
			}
		} catch (notificationUpdateError) {
		...
		try {
			...
			console.log('[USER][SOCIAL] Γ£à Real-time event emitted: follow_request:rejected to requester');
		} catch (realtimeError) {
			console.error('[USER][SOCIAL] Error emitting real-time event:', realtimeError);
			// Don't fail the request if real-time event fails
		}
