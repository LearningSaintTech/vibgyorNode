...
MessageRequestSchema.methods.accept = async function(responseMessage = '') {
	...
	try {
		...
		try {
			...
			console.log('[MESSAGE_REQUEST_MODEL] ΓÜá∩╕Å Cannot create session, using non-transactional mode:', sessionError.message);
		...
		const performAccept = async (sessionParam) => {
			...
			if (!chat) {
				console.error('[MESSAGE_REQUEST_MODEL] Γ¥î Chat.findOrCreateChat returned null');
				throw new Error('Failed to create chat - findOrCreateChat returned null');
			}
			
			console.log('[MESSAGE_REQUEST_MODEL] Γ£à Chat created:', chat._id);
			...
			console.log('[MESSAGE_REQUEST_MODEL] Γ£à Request and chat saved successfully');
			...
			if (requestMessage && requestMessage !== '') {
				try {
					...
					console.log('[MESSAGE_REQUEST_MODEL] Γ£à Initial message saved:', initialMessage._id);
					...
					console.log('[MESSAGE_REQUEST_MODEL] Γ£à Chat updated with initial message');
				} catch (error) {
					console.error('[MESSAGE_REQUEST_MODEL] ΓÜá∩╕Å Error creating initial message (non-fatal):', error.message);
					console.error('[MESSAGE_REQUEST_MODEL] ΓÜá∩╕Å Error stack:', error.stack);
		...
		if (useTransaction && session) {
			try {
				...
				console.log('[MESSAGE_REQUEST_MODEL] Γ£à Transaction completed successfully');
				...
				if (transactionError.message && (
					...
					console.log('[MESSAGE_REQUEST_MODEL] ΓÜá∩╕Å Transactions not supported, falling back to non-transactional mode');
		...
		if (!chat) {
			console.error('[MESSAGE_REQUEST_MODEL] Γ¥î Chat not found after operations:', chatId || this.chatId);
			throw new Error('Chat not found after completion');
		}
		
		console.log('[MESSAGE_REQUEST_MODEL] Γ£à Chat fetched successfully:', chat._id);
		return chat;
	} catch (error) {
		console.error('[MESSAGE_REQUEST_MODEL] Γ¥î Error in accept method:', error);
		console.error('[MESSAGE_REQUEST_MODEL] Γ¥î Error message:', error.message);
		console.error('[MESSAGE_REQUEST_MODEL] Γ¥î Error stack:', error.stack);
		console.error('[MESSAGE_REQUEST_MODEL] Γ¥î Request state:', {
			requestId: this._id,
			status: this.status,
