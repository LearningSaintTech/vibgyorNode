const { MessageRequest, Chat, User } = require('./messageRequest.repository');
	...
	try {
		...
		try {
			...
			try {
				...
				console.log('[MESSAGE_REQUEST] Γ£à Notification cache invalidated for recipient (new notification)');
			} catch (cacheError) {
		...
		try {
			...
			console.log('[MESSAGE_REQUEST] Γ£à Cache invalidated for recipient (new request)');
		} catch (cacheError) {
		...
		try {
			...
			if (!currentUser) {
				console.error('[MESSAGE_REQUEST] ΓÜá∩╕Å Current user not found for real-time event:', currentUserId);
			} else {
				...
				console.log('[MESSAGE_REQUEST] Γ£à Real-time event emitted: message_request:received to user', userId);
			}
		} catch (realtimeError) {
...
async function acceptMessageRequest(req, res) {
	try {
		...
		try {
			...
			if (!chat) {
				console.error('[MESSAGE_REQUEST] Γ¥î Chat is null after accept - transaction may have failed');
				return ApiResponse.serverError(res, 'Failed to create chat after accepting request');
			}
			console.log('[MESSAGE_REQUEST] Γ£à Chat created successfully:', chat._id);
		} catch (acceptError) {
			console.error('[MESSAGE_REQUEST] Γ¥î Error in request.accept():', acceptError);
			console.error('[MESSAGE_REQUEST] Γ¥î Error stack:', acceptError.stack);
			console.error('[MESSAGE_REQUEST] Γ¥î Request details:', {
				requestId: request._id,
				fromUserId: request.fromUserId,
				toUserId: request.toUserId,
				status: request.status,
		...
		try {
			...
			console.log('[MESSAGE_REQUEST] Γ£à Notification deleted for recipient after acceptance:', currentUserId);
		...
		try {
			...
			try {
				...
				console.log('[MESSAGE_REQUEST] Γ£à Notification cache invalidated for requester (acceptance notification)');
			} catch (cacheError) {
				console.error('[MESSAGE_REQUEST] Error invalidating notification cache:', cacheError);
			}
		} catch (notificationError) {
		...
		try {
			...
			console.log('[MESSAGE_REQUEST] Γ£à Cache invalidated for recipient and requester after acceptance');
		} catch (cacheError) {
			console.error('[MESSAGE_REQUEST] Error invalidating cache:', cacheError);
		...
		try {
			...
			console.log('[MESSAGE_REQUEST] Γ£à Real-time events emitted: message_request:accepted and chat:created');
		} catch (realtimeError) {
...
async function rejectMessageRequest(req, res) {
	try {
		...
		try {
			...
			console.log('[MESSAGE_REQUEST] Γ£à Notification deleted for recipient after rejection:', currentUserId);
		...
		try {
			...
			console.log('[MESSAGE_REQUEST] Γ£à Cache invalidated for recipient and requester');
		} catch (cacheError) {
			console.error('[MESSAGE_REQUEST] Error invalidating cache:', cacheError);
		...
		try {
			...
			console.log('[MESSAGE_REQUEST] Γ£à Real-time event emitted: message_request:rejected to requester');
		} catch (realtimeError) {
			console.error('[MESSAGE_REQUEST] Error emitting real-time event:', realtimeError);
	...
	try {
		...
		if (recipientId) {
			try {
				...
				console.log('[MESSAGE_REQUEST] Γ£à Notification deleted for recipient:', recipientId);
		...
		try {
			...
			console.log('[MESSAGE_REQUEST] Γ£à Cache invalidated for requester and recipient after deletion');
		} catch (cacheError) {
			console.error('[MESSAGE_REQUEST] Error invalidating cache:', cacheError);
		...
		if (recipientId) {
			try {
				...
				console.log('[MESSAGE_REQUEST] Γ£à Real-time event emitted: message_request:deleted to recipient');
			} catch (realtimeError) {
				console.error('[MESSAGE_REQUEST] Error emitting real-time event:', realtimeError);
				// Don't fail deletion if real-time event fails
			}
