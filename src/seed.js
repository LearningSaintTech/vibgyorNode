...
const connectDB = async () => {
  try {
    const mongoURI = "mongodb+srv://hraj6398_db_user:KIhzZWRFPMVk1Wie@cluster0.jq5gny8.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0";
  ...
  for (const chat of chats) {
    ...
    for (let i = 0; i < messageCount; i++) {
      ...
      const message = {
        ...
        senderId: sender,
        type: messageType,
        content: messageType === 'text' ? getRandomElement(demoData.messageContents) : undefined,
        media: (messageType === 'image' || messageType === 'audio') ? {
          url: `https://example.com/media/${Math.random().toString(36).substr(2, 9)}.${messageType === 'image' ? 'jpg' : 'mp3'}`,
          mimeType: messageType === 'image' ? 'image/jpeg' : 'audio/mpeg',
          fileName: `${messageType}_${i}.${messageType === 'image' ? 'jpg' : 'mp3'}`,
          fileSize: Math.floor(Math.random() * 1000000) + 100000,
          duration: messageType === 'audio' ? Math.floor(Math.random() * 300) + 10 : undefined,
